...
//                 {
//                    // Key_Buf[i].KEY_STATUS = KEY_NULL;//转入状态0
//                     Key_Buf[i].KEY_EVENT = KEY_UP;//空事件
//                 }
...
/*unsigned char GetKey()
{
    ...
    {
			 Key_Buf[KEY0].KEY_STATUS = KEY_NULL;
...
unsigned char GetKey()
{
   if(KEY0_ReadPin() == 0)//S4 按下
	 {
		  delay_ms(1);
      if(KEY0_ReadPin() == 0)
      {
        while(KEY0_ReadPin()==0);//等待按键释放
				return 1; 
      } 					 
   }
	 else if(KEY3_ReadPin() == 0)//S1 按下
	 {
		  delay_ms(1);
      if(KEY3_ReadPin() == 0)
      {
        while(KEY3_ReadPin()==0);//等待按键释放
				return 4; 
      } 					 
   }
	 return 0;
